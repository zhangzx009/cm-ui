...
test('simple columns confirm & cancel event', () => {
    ...
    wrapper.find('.cm-picker__confirm').trigger('click');
    wrapper.find('.cm-picker__cancel').trigger('click');
...
test('drag columns', () => {
    ...
    triggerDrag(wrapper.find('.cm-picker-column'), 0, -100);
    wrapper.find('.cm-picker-column ul').trigger('transitionend');
...
test('simulation finger swipe again before transitionend', () => {
    ...
    triggerDrag(wrapper.find('.cm-picker-column'), 0, -5);
    triggerDrag(wrapper.find('.cm-picker-column'), -5, -100);
    wrapper.find('.cm-picker-column ul').trigger('transitionend');
    expect(wrapper.emitted('change')[0][1]).toEqual('1995');
});
...
test('click column\'s item', () => {
    ...
    wrapper.findAll('.cm-picker-column__item').at(3).trigger('click');
